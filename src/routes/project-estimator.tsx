...
function ChoiceGrid({title,items,selected,onToggle,single=false}:{title:string;items:string[];selected:string[];onToggle:(item:string)=>void;single?:boolean}){return <div><p className="eyebrow mb-4">Estimator step</p><h2 className="fluid-h3">{title}</h2><div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{items.map((item)=><button type="button" key={item} aria-pressed={selected.includes(item)} onClick={()=>onToggle(item)} className={`rounded-2xl border p-5 text-left font-semibold transition ${selected.includes(item)?"border-brand-red bg-cream":"border-black/10 hover:border-black/25"}`}>{selected.includes(item)?<Check className="mb-3 h-5 w-5 text-brand-red"/>:null}{item}{single?null:<span className="mt-1 block text-xs font-normal text-ink-soft">Select or remove</span>}</button>)}</div></div>}
