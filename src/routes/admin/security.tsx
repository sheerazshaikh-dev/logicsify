  ...
  return <div className="space-y-6">
    <div className="grid gap-6 xl:grid-cols-2">
      <AdminCard className="p-6">
        ...
        {twoFactor.secret ? <div className="mt-5 rounded-xl bg-slate-50 p-4"><div className="grid gap-5 sm:grid-cols-[180px_1fr] sm:items-center">{twoFactor.uri?<QrCode value={twoFactor.uri} size={180} className="rounded-xl border border-slate-200 bg-white"/>:null}<div><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Scan with your authenticator app</p><p className="mt-2 text-xs leading-5 text-slate-500">Scan the QR code, then enter the current six-digit code above. If scanning is unavailable, use the manual key below.</p><p className="mt-3 break-all font-mono text-sm text-[#190A2F]">{twoFactor.secret}</p>{twoFactor.uri ? <a href={twoFactor.uri} className="mt-3 inline-flex text-xs font-semibold text-[#FE3434] hover:text-[#190A2F]">Open in an authenticator app</a> : null}</div></div></div> : null}
