...
export function SecurityPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSettings({ ...nextSettings, legacy_admin_path_enabled: false });
  ...
  async function save() {
    ...
    try {
      const result = await saveSecuritySettings({
        ...settings,
        legacy_admin_path_enabled: false,
      });
  ...
  return (
    <AdminShell>
      ...
      <div className="mb-6 flex flex-wrap gap-2">{tabs.map((item) => <button key={item.id} onClick={() => setTab(item.id)} className={`rounded-xl px-4 py-2.5 text-sm font-semibold ${tab === item.id ? "bg-ink text-white" : "border border-slate-200 bg-white text-slate-600"}`}>{item.label}</button>)}</div>
...
function Overview({ overview, events, refresh }: { overview: SecurityOverview; events: AuditLog[]; refresh: () => void }) {
  ...
  return (
    <div className="space-y-6">
        ...
        {metrics.map(([label, value, Icon]) => (
          <AdminCard key={label} className="p-5">
            <div className="flex items-center justify-between">
              ...
              <span className="grid h-11 w-11 place-items-center rounded-xl bg-slate-100 text-ink"><Icon className="h-5 w-5" /></span>
            </div>
          </AdminCard>
        ))}
      </div>
      ...
      {overview.locked_administrators?.length ? (
        <AdminCard>
          <div className="border-b border-slate-200 p-5">
            <h2 className="font-semibold text-ink">Locked administrator accounts</h2>
      ...
      <AdminCard>
        <div className="border-b border-slate-200 p-5"><h2 className="font-semibold text-ink">Recent security activity</h2></div>
  ...
  return <div className="space-y-6">
    <div className="grid gap-6 xl:grid-cols-2">
      <AdminCard className="p-6">
        <div className="flex items-start gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-ink text-white"><Smartphone className="h-5 w-5" /></span><div><h2 className="text-lg font-semibold text-ink">Authenticator app</h2><p className="mt-1 text-sm text-slate-500">Status: <strong>{settings.two_factor_authenticator_enabled ? "Enabled" : "Not enabled"}</strong>. Works with Google Authenticator, Microsoft Authenticator, Authy, 1Password, and other TOTP apps.</p></div></div>
        ...
        {twoFactor.secret ? <div className="mt-5 rounded-xl bg-slate-50 p-4"><div className="grid gap-5 sm:grid-cols-[180px_1fr] sm:items-center">{twoFactor.uri?<QrCode value={twoFactor.uri} size={180} className="rounded-xl border border-slate-200 bg-white"/>:null}<div><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Scan with your authenticator app</p><p className="mt-2 text-xs leading-5 text-slate-500">Scan the QR code, then enter the current six-digit code above. If scanning is unavailable, use the manual key below.</p><p className="mt-3 break-all font-mono text-sm text-ink">{twoFactor.secret}</p>{twoFactor.uri ? <a href={twoFactor.uri} className="mt-3 inline-flex text-xs font-semibold text-brand-red hover:text-ink">Open in an authenticator app</a> : null}</div></div></div> : null}
      ...
      <AdminCard className="p-6">
        <div className="flex items-start gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-lavender text-brand-red"><Mail className="h-5 w-5" /></span><div><h2 className="text-lg font-semibold text-ink">Email verification</h2><p className="mt-1 text-sm text-slate-500">Status: <strong>{settings.two_factor_email_enabled ? "Enabled" : "Not enabled"}</strong>. Codes are sent to {settings.two_factor_email_address || "your administrator email"} and expire after 10 minutes.</p></div></div>
