...
import { ArrowRight, CheckCircle2, Download, FileText, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/guides/$slug")({
  ...
  loader: async ({ params }) => {
    const [resource, all] = await Promise.all([
      getCmsContentItem("resource", params.slug),
      getCmsContentList("resource"),
    ]);
    if (!resource) throw notFound();
    return {
      resource,
      related: all.filter((item) => item.slug !== params.slug).slice(0, 3),
    };
  },
  component: GuidePage,
    ...
    meta: [
      {
        title:
          ...
          `${loaderData?.resource.title || "Guide"} | Logicsify`,
      },
      {
        name: "description",
        content:
          loaderData?.resource.seo_json?.description || loaderData?.resource.excerpt || "",
      },
      {
        property: "og:title",
        content: loaderData?.resource.seo_json?.title || loaderData?.resource.title || "",
      },
      {
    ...
    links: [
      {
        rel: "canonical",
        href:
          loaderData?.resource.seo_json?.canonical ||
...
function GuidePage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const result = await requestResourceDownload({ resource_slug: resource.slug, ...form });
      setDownload(result.download_url);
      trackEvent("resource_form_submitted", { slug: resource.slug });
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not prepare the guide.");
    } finally {
      setLoading(false);
    }
  }
  ...
  function beginDownload() {
    trackEvent("resource_downloaded", { slug: resource.slug });
  ...
  return (
    <SiteLayout>
      <PageHero
        eyebrow={String(content.category || "Guide")}
        breadcrumbs={[
          { label: "Home", to: "/" },
            ...
            <figure className="overflow-hidden rounded-3xl border border-black/10 bg-cream shadow-[var(--shadow-card)]">
              <div className="aspect-[16/7] w-full overflow-hidden">
                <img
                  ...
                  alt={String(content.featured_image_alt || resource.title)}
                  className="h-full w-full object-cover"
                  decoding="async"
                  fetchPriority="high"
                />
              </div>
              {content.featured_image_caption ? (
                <figcaption className="border-t border-black/10 px-5 py-3 text-sm text-ink-soft">
                  {String(content.featured_image_caption)}
                </figcaption>
              ) : null}
            </figure>
          </div>
          ...
          <article className="min-w-0 lg:col-span-7">
            ...
            {Array.isArray(content.includes) ? (
              <section className="mt-12">
                <h2 className="fluid-h3">What is included</h2>
                <ul className="mt-5 space-y-3">
                  {content.includes.map((item) => (
                    <li key={String(item)} className="flex gap-3 text-ink-soft">
            ...
            {content.audience ? (
              <section className="mt-10 rounded-2xl bg-cream p-6">
                ...
                <p className="mt-3 text-ink-soft">{String(content.audience)}</p>
              </section>
            ) : null}
          </article>

          <aside className="lg:col-span-5">
            <div className="rounded-3xl border border-black/10 bg-white p-6 shadow-[var(--shadow-card)] lg:sticky lg:top-28">
              <h2 className="text-xl font-semibold">Request this guide</h2>
              <p className="mt-2 text-sm text-ink-soft">
                The file link is returned only after a valid lead submission.
              </p>
              {download ? (
                <div className="mt-6 rounded-2xl bg-green-50 p-5">
                  <CheckCircle2 className="h-6 w-6 text-green-700" />
                  <p className="mt-3 text-sm text-green-800">Your guide is ready.</p>
                  <button onClick={beginDownload} className="btn-primary mt-4">
      ...
      {related.length ? (
        <section className="bg-cream py-20">
          <div className="container-page">
            <h2 className="fluid-h3">Related guides</h2>
              ...
              {related.map((item) => (
                ...
                >
                  {item.featured_image ? (
                    ...
                    <div className="grid aspect-[16/9] place-items-center bg-white/60">
                      <FileText className="h-10 w-10 text-ink-soft/50" />
                    </div>
                  )}
                  <div className="p-6">
