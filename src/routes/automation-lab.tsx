...
function CrmDemo() {
  ...
  return <div>
    <div className="flex flex-wrap gap-2"><SiteSelect value="" onValueChange={(value)=>{if(value)setNodes([...nodes,value])}} options={pool} placeholder="Add workflow node" tone="light" className="max-w-xs" /><button onClick={run} className="btn-primary"><Play className="h-4 w-4"/>Run demo</button><button onClick={()=>{setNodes(["New website lead","Score","Create opportunity","Assign owner","Schedule follow-up"]);setActive(-1);trackEvent("automation_demo_reset",{demo:"crm"})}} className="btn-ghost-light"><RotateCcw className="h-4 w-4"/>Reset</button></div>
...
function Select({label,value,options,onChange}:{label:string;value:string;options:string[];onChange:(value:string)=>void}){return <label className="block"><span className="mb-2 block text-sm font-semibold text-ink">{label}</span><SiteSelect value={value} onValueChange={onChange} options={options} tone="light" /></label>}
