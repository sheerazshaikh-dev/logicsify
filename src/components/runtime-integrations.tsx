...
import { getPublicIntegrations, type PublicIntegrations } from "@/lib/logicsify-api";
import { injectCodeSnippets, removeRuntimeNamespace } from "@/lib/runtime-code";
...
function addLegacyHtml(key: string, html: string | undefined, target: HTMLElement) {
...
function scheduleAnalytics(callback: () => void) {
  let completed = false;
  ...
  const fallbackTimer = window.setTimeout(run, 12_000);
  return cleanup;
}

function loadBuiltInAnalytics(settings: PublicIntegrations) {
  ...
  if (gtm) {
    ...
    // When GTM is configured it owns GA4. Loading both directly duplicates the
    // Google tag library and was the largest third-party PageSpeed cost.
    addScript("ga4-lib", `https://www.googletagmanager.com/gtag/js?id=${encodeURIComponent(ga4)}`);
    addScript(
      "ga4-config",
      undefined,
...
export function RuntimeIntegrations() {
  ...
  useEffect(() => {
    if (location.pathname.startsWith("/admin") || location.pathname.startsWith("/control/")) {
      removeRuntimeNamespace("public-snippet");
      return;
    }
    ...
    getPublicIntegrations().then((settings) => {
      ...
      if (trackingEnabled) {
        cancelScheduledAnalytics = scheduleAnalytics(() => loadBuiltInAnalytics(settings));
      ...
      injectCodeSnippets(settings.snippets, "public", "public-snippet");
      addLegacyHtml("custom-head", settings.head_code, document.head);
      addLegacyHtml("custom-body", settings.body_code, document.body);
    });
